...
function info () {
  ...
  args.unshift('%cpanist-web-extension: %c%s', 'color:#337ab7;font-weight:bold;', 'color: #51515d;');
...
function debug (message) {
  if (!(config && config.mustDebug)) return;
  warn('panist-web-extension: ' + message);
}

function logXhrError (url, statusText) {
  console.error('%cpanist-web-extension:%c %s %c %s',
